...
async function run() {
  const { data: stays, error: staysError } = await supabase.from('stays').select('id, title, host_id').limit(5);
  console.log("Stays:", stays || [], staysError || "");

  const { data: profiles, error: profilesError } = await supabase.from('profiles').select('id, full_name').limit(5);
