...
export function AddHotelForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || 'Failed to create hotel listing');
    }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate('/host/hotels')}><ArrowLeft className="h-5 w-5" /></Button>
        <div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div><Label>Hotel Name *</Label><Input value={form.title} onChange={e => setF('title', e.target.value)} placeholder="e.g. Grand Palace Hotel" /></div>
              ...
              <div><Label>Full Description</Label><Textarea value={form.description} onChange={e => setF('description', e.target.value)} placeholder="Describe your hotel in detail..." rows={5} /></div>
              <div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div><Label>Location *</Label><Input value={form.location} onChange={e => setF('location', e.target.value)} placeholder="e.g. Connaught Place, New Delhi" /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><Label>State</Label><Input value={form.state} onChange={e => setF('state', e.target.value)} placeholder="e.g. Delhi" /></div>
                <div><Label>City</Label><Input value={form.city} onChange={e => setF('city', e.target.value)} placeholder="e.g. New Delhi" /></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Latitude</Label><Input type="number" step="any" value={form.latitude} onChange={e => setF('latitude', e.target.value)} placeholder="28.6139" /></div>
                <div><Label>Longitude</Label><Input type="number" step="any" value={form.longitude} onChange={e => setF('longitude', e.target.value)} placeholder="77.2090" /></div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div><Label>Max Guests *</Label><Input type="number" min={1} value={form.max_guests} onChange={e => setF('max_guests', e.target.value)} placeholder="100" /></div>
              <div><Label>Rooms / Bedrooms</Label><Input type="number" min={0} value={form.bedrooms} onChange={e => setF('bedrooms', e.target.value)} placeholder="50" /></div>
              <div><Label>Beds</Label><Input type="number" min={0} value={form.beds} onChange={e => setF('beds', e.target.value)} placeholder="60" /></div>
              <div><Label>Bathrooms</Label><Input type="number" min={0} value={form.bathrooms} onChange={e => setF('bathrooms', e.target.value)} placeholder="50" /></div>
              <div className="md:col-span-2"><Label>Property Size</Label><Input value={form.propertySize} onChange={e => setF('propertySize', e.target.value)} placeholder="e.g. 5000 sq ft" /></div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <ListingImageUploader images={form.images} onImagesChange={v => setF('images', v)} label="Click to upload hotel photos" showFeaturedBadge />
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <ListingImageUploader images={form.roomImages} onImagesChange={v => setF('roomImages', v)} label="Click to upload room / interior photos" showFeaturedBadge={false} />
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-5">
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <Input value={form.amenityInput} onChange={e => setF('amenityInput', e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addCustomAmenity(); } }} placeholder="e.g. Spa, Conference Hall, Valet Parking" />
          ...
          {/* Room Types / Sleeping Arrangements */}
          <Card>
              ...
              {form.bedroomDetails.map((b, i) => (
                <div key={i} className="grid grid-cols-3 gap-3 p-3 rounded-lg border border-border">
                  <div>
                    ...
                    <Input value={b.name} onChange={e => setForm(p => ({ ...p, bedroomDetails: p.bedroomDetails.map((bd, idx) => idx === i ? { ...bd, name: e.target.value } : bd) }))} placeholder="e.g. Deluxe Room" className="mt-1 h-8 text-sm" />
                  </div>
                  <div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div><Label>House Rules</Label><Textarea value={form.houseRules} onChange={e => setF('houseRules', e.target.value)} placeholder="No smoking in rooms, ID required at check-in..." rows={3} /></div>
              <div><Label>Health & Safety</Label><Textarea value={form.healthSafety} onChange={e => setF('healthSafety', e.target.value)} placeholder="Smoke detectors, CCTV, 24/7 security..." rows={3} /></div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div><Label>Price per Night *</Label><Input type="number" min={1} value={form.price_per_night} onChange={e => setF('price_per_night', e.target.value)} placeholder="5000" /></div>
              ...
              {form.coupons.map((c, i) => (
                <div key={i} className="p-3 rounded-lg border border-border space-y-2">
                  <div className="flex gap-2">
                    <Input value={c.code} onChange={e => setForm(p => ({ ...p, coupons: p.coupons.map((cp, idx) => idx === i ? { ...cp, code: e.target.value.toUpperCase() } : cp) }))} placeholder="SAVE20" className="uppercase flex-1 h-8 text-sm" />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <Input type="number" min={1} value={c.value} onChange={e => setForm(p => ({ ...p, coupons: p.coupons.map((cp, idx) => idx === i ? { ...cp, value: e.target.value } : cp) }))} placeholder={c.type === 'percent' ? '20' : '500'} className="h-8 text-sm" />
                  </div>
                </div>
              ))}
            </CardContent>
